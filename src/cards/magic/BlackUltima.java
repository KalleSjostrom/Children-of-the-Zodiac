 ...
 */
public class BlackUltima extends Card {
	...
	public ParticleSystemPacket getAnimation(AnimationInfo info) {
		return BlackUltima.getStaticAnimation(info);
	}
	...
	public static ParticleSystemPacket getStaticAnimation(AnimationInfo info) {
		...
		settings.setVector(AnimSettings.TARGET, randomTarget(target));
	...
	private static Vector3f randomTarget(Vector3f t) {
		Vector3f newtarget = new Vector3f(t);
		t.y = (float) Math.random() - .5f;
		t.x = (float) Math.random() - .5f;
		return newtarget;
	}
}
