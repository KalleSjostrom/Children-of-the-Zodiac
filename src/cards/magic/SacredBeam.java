 ...
 */
public class SacredBeam extends Card {
	...
	public ParticleSystemPacket getAnimation(AnimationInfo info) {
		return SacredBeam.getStaticAnimation(info);
	}
	...
	public static ParticleSystemPacket getStaticAnimation(AnimationInfo info) {
		...
		if (!info.isOnEnemy()) {
			...
			settings.setVector(AnimSettings.TARGET, new Vector3f(0, -.5f, -9f));
			settings.setValue(AnimSettings.DESTROY_MODE, ParticleValues.DESTROY_MULTIPLE_TIMES);
			settings.setValue(AnimSettings.COLOR, ParticleValues.COLOR_RANDOM);
		}
		
		settings.setValue(AnimSettings.SIZE, .3f);
