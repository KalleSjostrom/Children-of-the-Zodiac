...
public class BlackWind extends Magic {
	...
	public ParticleSystemPacket getAnimation(AnimationInfo info) {
		...
		return BlackWind.getStaticAnimation(info);
	}
...
//		int level = new Random().nextInt(3) + 1;
		...
		def.setValue(AnimSettings.SPEED, 7f);
		def.setBoolean(AnimSettings.ADDITIVE_COLORING, false);
		if (info.isOnEnemy()) {
		...
		int color = ParticleValues.COLOR_BLACK;
