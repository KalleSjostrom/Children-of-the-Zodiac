...
public class Cure extends SupportCard {
	...
	private static final String[] SOUNDS = {SoundMap.MAGICS_CURE};
		...
		if (!info.isOnEnemy()) {
			if (info.isOnAll()) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (c.isAlive()) {
						addTargetToSettings(i, settings);
						addToSystemPacket(psp, settings);
					}
				}
			} else {
				addTargetToSettings(info.getTarget(), settings);
				addToSystemPacket(psp, settings);
			}
		} else {
			float x = -.4f;
	...
	public static void addToSystemPacket(ParticleSystemPacket psp,
		...
		settings.setValue(AnimSettings.FADE_RANDOM_MULT, .02f);
		settings.setValue(AnimSettings.FADE_RANDOM_ADD, .02f);
